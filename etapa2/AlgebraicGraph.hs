...
outNeighbors node (Connect a b) 
    | elem node (nodes a) = S.union (S.union (outNeighbors node a) (outNeighbors node b)) (nodes b)
    | otherwise = S.union (S.union (outNeighbors node a) (outNeighbors node b)) (outNeighbors node b)
...
inNeighbors node (Connect a b) 
    | elem node (nodes b) = S.union (S.union (inNeighbors node a) (inNeighbors node b)) (nodes a)
    | otherwise = S.union (S.union (inNeighbors node a) (inNeighbors node b)) (inNeighbors node a)
...
splitNode old news (Node a)
    | length news == 0 && a == old = removeNode old (Node a)
    | a == old = Overlay (Node (head news)) (splitNode old (tail news) (Node a))
    | otherwise = Node a
