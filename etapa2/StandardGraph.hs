...
outNeighbors node graph = S.map (\x -> snd x) (S.filter (\x -> fst x == node) (edges graph))
...
inNeighbors node graph = S.map (\x -> fst x) (S.filter (\x -> snd x == node) (edges graph))
...
removeNode node graph = (S.filter (\x -> node /= x) (nodes graph), S.filter (\x -> (snd x /= node) && (fst x /= node)) (edges graph))
...
splitNode old news graph = removeNode old (S.delete old (S.union (S.fromList news) (nodes graph)), 
            S.union (S.union (S.cartesianProduct (S.fromList news) (outNeighbors old graph)) 
...
mergeNodes prop node graph = (
    if null (S.filter prop (nodes graph)) 
        then (nodes graph)
    else (S.insert node (S.difference (nodes graph) (S.filter prop (nodes graph)))),
    S.map (\x -> 
                if S.member (fst x) (S.filter prop (nodes graph)) && S.member (snd x) (S.filter prop (nodes graph))
                    then (node, node)
                else if S.member (snd x) (S.filter prop (nodes graph)) 
                    then (fst x, node)
                else if S.member (fst x) (S.filter prop (nodes graph))
                    then (node, snd x)
                else (fst x, snd x)) 
                (edges graph))